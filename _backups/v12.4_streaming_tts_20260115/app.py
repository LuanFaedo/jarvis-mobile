...
# Tenta importar o Módulo de Automação (God Mode) apenas se houver tela
pc = None
try:
    # A verificação de DISPLAY é crucial em Linux para evitar erros em ambientes headless
    if os.environ.get('DISPLAY'):
        from sistema.automacao import pc
    else:
        print("[AVISO] Nenhuma tela (DISPLAY) detectada. O módulo de automação (God Mode) não será carregado.")
except Exception as e:
    print(f"[AVISO] Falha ao importar 'sistema.automacao': {e}. O God Mode fica desativado.")
...
def processar_comandos_sistema(resposta_llm, user_id, profundidade=0):
    ...
    for acao_raw in autos:
        if not pc:
            output_extra += "\n> [ERRO AUTO]: Módulo de automação não carregado (sem tela).\n"
            continue
        try:
...
"""
    ...
    if os.environ.get('DISPLAY') and (any(k in texto_lower for k in ["veja minha tela", "olhe minha tela", "o que está na tela", "leia a tela", "analise a tela"]) or modo_leitura_texto and "tela" in texto_lower):
...
# ============================================================
# STREAMING TTS EM TEMPO REAL (NOVA IMPLEMENTAÇÃO)
# ============================================================

def stream_llm_sentences(messages: list, model: str = None):
    """
    Generator que faz streaming do LLM e retorna sentenças completas.
    Usa yield para entregar cada frase assim que detecta pontuação final.

    Yields:
        str: Sentença completa (termina com . ? ! ou \n)
    """
    modelo = model or MODELO_ATIVO
    frase_buffer = ""
    delimitadores = ('.', '?', '!', '\n')
    ...
    try:
        ...
        for chunk in stream:
            ...
            # Verifica se temos uma sentença completa
            while any(d in frase_buffer for d in delimitadores):
                # Encontra o primeiro delimitador
                pos = -1
                for d in delimitadores:
                    idx = frase_buffer.find(d)
                ...
                # Extrai a sentença até o delimitador (inclusive)
                sentenca = frase_buffer[:pos + 1].strip()
                frase_buffer = frase_buffer[pos + 1:]

                if len(sentenca) > 3:  # Ignora sentenças muito curtas
                    yield sentenca

        # Envia o resto do buffer
        if frase_buffer.strip() and len(frase_buffer.strip()) > 3:
            yield frase_buffer.strip()

    except Exception as e:
        print(f"[STREAM LLM] Erro: {e}", flush=True)
        yield f"Erro no streaming: {e}"


async def stream_tts_audio_async(text: str):
    """
    Async generator que converte texto em chunks de áudio MP3 via edge-tts.
    Usa streaming nativo do edge-tts para mínima latência.

    Yields:
        bytes: Chunks de áudio MP3
    """
    try:
        # Lê configuração de velocidade
        try:
            with open(CONFIG_FILE, 'r') as f:
                speed = json.load(f).get('velocidade', '+20%')
        except:
            speed = '+20%'

        # Limpa texto para TTS
        text_limpo = text
        for tag in ["[SISTEMA]", "[CMD]", "SISTEMA:", "Traceback", "Error:"]:
            text_limpo = text_limpo.replace(tag, "")
        text_limpo = re.sub(r'```.*?```', '', text_limpo, flags=re.DOTALL)
        text_limpo = re.sub(r'\[\[.*?\]\]', '', text_limpo)
        text_limpo = text_limpo.strip()
        ...
        # Edge-TTS com streaming nativo
        communicate = edge_tts.Communicate(text_limpo, "pt-BR-AntonioNeural", rate=speed)
...
def stream_tts_audio_sync(text: str):
    """
    Versão síncrona do stream_tts_audio.
    Retorna lista de chunks de áudio.
    """
    async def collect_chunks():
        chunks = []
        async for chunk in stream_tts_audio_async(text):
            chunks.append(chunk)
        return chunks
...
def stream_text_to_audio(text_generator, socketio_instance, room_sid: str, user_id: str = "Mestre"):
    """
    Função principal de streaming TTS em tempo real.

    1. Recebe um generator de sentenças do LLM
    2. Para cada sentença, gera áudio via edge-tts streaming
    3. Emite chunks de áudio via Socket.IO IMEDIATAMENTE

    Args:
        text_generator: Generator que produz sentenças (strings)
        socketio_instance: Instância do SocketIO
        room_sid: ID da sala/sessão do cliente
        user_id: ID do usuário para salvar histórico
    """
    texto_completo = ""
    sentenca_idx = 0

    def processar_sentenca(sentenca: str, idx: int):
        """Processa uma sentença em thread separada"""
        try:
            # Emite texto parcial imediatamente
            socketio_instance.emit('bot_msg_partial', {'data': sentenca + " "}, room=room_sid)

            # Gera áudio em chunks
            audio_chunks = stream_tts_audio_sync(sentenca)

            if audio_chunks:
                # Combina todos os chunks em um único áudio
                audio_completo = b''.join(audio_chunks)
                audio_b64 = base64.b64encode(audio_completo).decode('utf-8')

                # Emite o áudio da sentença
                socketio_instance.emit('stream_audio_chunk', {
                    'audio': audio_b64,
                    'index': idx,
                    'text': sentenca,
                    'format': 'mp3'
                ...
                print(f"[STREAM] Sentença {idx}: {sentenca[:30]}... ({len(audio_b64)} bytes)", flush=True)

        except Exception as e:
            print(f"[STREAM] Erro na sentença {idx}: {e}", flush=True)

    try:
        for sentenca in text_generator:
            texto_completo += sentenca + " "

            # Processa cada sentença em thread para não bloquear
            # Mas mantemos ordem com índice
            thread = threading.Thread(
                target=processar_sentenca,
                args=(sentenca, sentenca_idx),
                daemon=True
            )
            thread.start()
            thread.join(timeout=10)  # Aguarda no máximo 10s por sentença

            sentenca_idx += 1

        # Sinaliza fim do streaming
        socketio_instance.emit('bot_msg_end', {'full_text': texto_completo.strip()}, room=room_sid)

        # Salva no histórico
        adicionar_mensagem(user_id, "assistant", texto_completo.strip())

        return texto_completo.strip()

    except Exception as e:
        print(f"[STREAM] Erro geral: {e}", flush=True)
        socketio_instance.emit('bot_msg', {'data': f"Erro no streaming: {e}"}, room=room_sid)
        return ""


def processar_com_streaming_real(user_id: str, texto: str, sid: str):
    """
    Handler completo para processamento com streaming real.
    Substitui o fluxo antigo com latência otimizada.
    ...
    """
    print(f"[STREAM REAL] Iniciando para {user_id}: {texto[:50]}...", flush=True)
    ...
    # Monta contexto (versão simplificada para velocidade)
    usuario = get_ou_criar_usuario(user_id)
    buffer_msgs = get_ultimas_mensagens(user_id, BUFFER_SIZE)
    saldo_atual = get_saldo(user_id)
    fatos = get_fatos(user_id)
...
SALDO: R$ {saldo_atual:.2f}"""
    ...
    # Cria generator de sentenças
    sentence_generator = stream_llm_sentences(messages)

    # Processa com streaming real
    resposta = stream_text_to_audio(sentence_generator, socketio, sid, user_id)

    # Processa comandos do sistema se houver
    if "[[" in resposta and "]]" in resposta:
        output_sys = processar_comandos_sistema(resposta, user_id)
        if output_sys:
...
def capturar_tela_base64():
    if not os.environ.get('DISPLAY'):
        print("[ERRO SCREENSHOT] Nenhuma tela detectada.")
        return None
    try:
        screenshot = ImageGrab.grab()
        buffered = io.BytesIO()
...
@socketio.on('audio_mobile')
def handle_audio_mobile(data):
    user_id = data.get('user_id', 'AndroidClient')
    audio_b64 = data.get('audio', '')
    formato = data.get('formato', 'm4a') # Assume m4a se nao especificado
    sid = request.sid

    if not audio_b64:
        print(f"[AUDIO MOBILE] Erro: Audio vazio para {user_id}", flush=True)
        socketio.emit('bot_msg', {'data': "SISTEMA: Erro - nenhum áudio recebido."}, room=sid)
        return
    
    # Salvar áudio original (opcional, para debug/registro)
    threading.Thread(target=salvar_diario_voz, args=(audio_b64, user_id, f"mobile_{formato}"), daemon=True).start()

    # 1. Transcrever áudio
    transcricao = transcrever_audio(audio_b64)
    if "[ERRO" in transcricao or "[Áudio inaudível]" in transcricao:
        print(f"[AUDIO MOBILE] Transcricao falhou: {transcricao}", flush=True)
        socketio.emit('bot_msg', {'data': f"SISTEMA: Não entendi o que você disse. {transcricao}"}, room=sid)
        registrar_input_usuario(f"ERRO: {transcricao}") # Registra o erro de transcrição
        return

    print(f"[AUDIO MOBILE] Audio transcrito para: {transcricao[:50]}...", flush=True)
    texto_processado = transcricao
    
    # 1. Auditoria Imediata (Segurança de Dados)
    registrar_input_usuario(texto_processado)
    print(f"[AUDITORIA] Input registrado: {texto_processado[:50]}...", flush=True)
    ...
    def processar_streaming(room_sid):
        ...
        if detectar_pergunta_horario(texto_processado) or \
           ...
           any(k in texto_lower for k in ["espelhar celular", "limpar memoria"]):
            ...
            res = gerar_resposta_jarvis(user_id, texto_processado)
...
"""
        ...
        try:
            ...
            adicionar_mensagem(user_id, "user", texto_processado)
...
def handle_audio_stream(data):
    """
    Novo endpoint OTIMIZADO para o cliente Flutter.
    Recebe bytes brutos (não base64) para menor latência.
    """
    sid = request.sid
    print(f"[FLUTTER] Recebidos {len(data)} bytes de áudio.", flush=True)

    def processar_flutter():
        temp_input = None
        temp_wav = None
        try:
            # 1. Salvar Bytes em Arquivo Temporário
            # O plugin 'record' do Flutter geralmente envia m4a/aac por padrão
            with tempfile.NamedTemporaryFile(suffix=".m4a", delete=False) as t:
                t.write(data)
                temp_input = t.name
            
            # 2. Converter para WAV (Obrigatório para speech_recognition)
            ...
            print(f"[FLUTTER] Transcrição: {texto}", flush=True)

            # 4. Lógica WAKE WORD (Prioridade Máxima)
            palavras_gatilho = ["jarvis", "jarvis", "jardis", "service", "start"]
            texto_lower = texto.lower()
            
            # Se detectou a palavra mágica
            if any(gatilho in texto_lower for gatilho in palavras_gatilho):
                print("[FLUTTER] Wake Word detectada! Enviando ACK.", flush=True)
                socketio.emit('jarvis_ack', {'status': 'listening'}, room=sid)
            
            # 5. Processamento Normal do Jarvis
            # Registra auditoria
            threading.Thread(target=registrar_input_usuario, args=(texto,), daemon=True).start()

            # Gera resposta LLM
            # user_id fixo por enquanto ou poderia vir num wrapper json
            user_id = "Mestre" 
            resposta_texto = gerar_resposta_jarvis(user_id, texto)
            
            # 6. Gera Áudio da Resposta
            # Usa a mesma função otimizada de áudio
            audio_b64 = gerar_audio_b64(resposta_texto)
            
            if audio_b64:
                socketio.emit('response_audio', {'audio': audio_b64, 'text': resposta_texto}, room=sid)
            else:
                 socketio.emit('bot_msg', {'data': resposta_texto}, room=sid)

        except sr.UnknownValueError:
            print("[FLUTTER] Áudio não entendido.", flush=True)
            socketio.emit('bot_msg', {'data': "?"}, room=sid)
        except Exception as e:
            print(f"[FLUTTER] Erro: {e}", flush=True)
            socketio.emit('bot_msg', {'data': f"Erro no servidor: {e}"}, room=sid)
        finally:
            # Limpeza
    ...
    threading.Thread(target=processar_flutter, daemon=True).start()
...
@socketio.on('fala_usuario_v2')
def handle_web_streaming_real(data):
    """
    NOVO ENDPOINT: Streaming TTS em tempo real otimizado.
    Usa edge-tts streaming nativo para mínima latência.

    Eventos emitidos:
    - 'bot_msg_partial': Texto parcial (cada token)
    - 'stream_audio_chunk': Áudio de cada sentença
    - 'bot_msg_end': Fim do streaming
    """
    user_id = data.get('user_id', 'Patrick')
    texto = data.get('text', '')
    sid = request.sid
    ...
    # Processa em thread para não bloquear
    threading.Thread(
        target=processar_com_streaming_real,
        args=(user_id, texto, sid),
        daemon=True
    ).start()


@socketio.on('audio_stream_v2')
def handle_audio_streaming_real(data):
    """
    NOVO ENDPOINT: Áudio + Streaming TTS em tempo real.
    Recebe áudio, transcreve e responde com streaming.
    """
    sid = request.sid
    ...
    if not audio_b64:
        socketio.emit('bot_msg', {'data': "Erro: áudio vazio"}, room=sid)
        return

    def processar():
        # 1. Transcreve
        texto = transcrever_audio(audio_b64)
        if not texto or "[ERRO" in texto or "[Áudio" in texto:
            socketio.emit('bot_msg', {'data': f"Não entendi: {texto}"}, room=sid)
            return

        print(f"[AUDIO V2] Transcrito: {texto}", flush=True)
        socketio.emit('transcription', {'text': texto}, room=sid)

        # 2. Processa com streaming real
        processar_com_streaming_real(user_id, texto, sid)
...
if __name__ == '__main__':
    ...
    try:
        from pycloudflared import try_cloudflare
        # Tenta na porta 5001
        public_url_obj = try_cloudflare(port=5001)
    ...
    def start_zap():
        ...
        if os.path.exists(zap_path):
            if sys.platform == "win32":
                ...
                # For Linux and other OSes. We run it in the background.
                subprocess.Popen(f'cd "{zap_path}" && npm start &', shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
